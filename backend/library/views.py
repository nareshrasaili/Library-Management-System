...
def librarian_login(request):
    if request.method == "POST":
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT password FROM librarian WHERE email = %s",
                [email]
            )
            row = cursor.fetchone()
...
def add_book(request):
    if request.method == "POST":
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT book_id FROM books WHERE book_number = %s",
                [book_number]
            )
            if cursor.fetchone():
            ...
            if author:
                ...
                cursor.execute(
                    "INSERT INTO author (author_name) VALUES (%s) RETURNING author_id",
                    [author_name]
                )
            ...
            cursor.execute(
                """
                INSERT INTO books 
                (title, total_copies, available_copies, author_id, book_number)
                VALUES (%s, %s, %s, %s, %s)
                """,
...
def add_student(request):
    if request.method == "POST":
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT user_id FROM users WHERE roll_no = %s",
                [roll_no]
            )
            if cursor.fetchone():
            ...
            cursor.execute(
                """
                INSERT INTO users (name, roll_no, batch, faculty, email)
                VALUES (%s, %s, %s, %s, %s)
                """,
...
def search_student(request):
    if request.method == "GET":
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT name, roll_no, batch, faculty, email FROM Users WHERE roll_no = %s",
                [roll_no]
            )
            row = cursor.fetchone()
...
def return_book(request):
    if request.method == "POST":
        ...
        roll_no = data.get("roll_no")
        book_number = data.get("book_number")
        return_date = data.get("return_date")  # optional: from frontend
        ...
        try:
            with transaction.atomic():
                with connection.cursor() as cursor:
                    ...
                    # Check if the book is actually issued to the student
                    cursor.execute(
                    ...
                    # ✅ Update issued_book to mark it as returned
                    cursor.execute(
